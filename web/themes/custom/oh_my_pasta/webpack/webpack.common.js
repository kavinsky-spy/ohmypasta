...
const mapFilenamesToEntries = (pattern) =>
  glob.sync(pattern).reduce((entries, filename) => {
    const [, name] = filename.match(/([a-zA-Z0-9_-]+)\.scss$/);
...
function getEntries(pattern) {
  const entries = {
    ...
    ...mapFilenamesToEntries(path.resolve('components/06-misc/**/*.scss')), // Drupal components.
  };

  glob.sync(pattern).forEach((file) => {
    const filePath = file.split(/components(.*)/s)[1];
